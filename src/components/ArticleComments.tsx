...
export default function ArticleComments({ article }: { article: any }) {
  ...
  const { comments, contentLoading, insertCommentLoading } = useAppSelector((state) => state.comments)
  ...
  useEffect(() => {
    ...
    dispatch(fetchCommentsThunk({ articleId: article.id, imageId: null }))
  }, [article?.id, dispatch])
  ...
  return (
    <div className="border p-3 mb-4">
      ...
      {contentLoading && <p className="text-muted">Loading comments…</p>}
      ...
      {comments.length === 0 && !contentLoading && <p className="text-muted">No comments yet.</p>}
    </div>
  )
}
